...
function indexInject() {
  return gulp
    ...
    .pipe(
      inject(gulp.src(["build/*.html", "!build/index.html"], { read: false }), {
        transform: function (filepath) {
          filepathFilter = filepath.replace("/build/", "");
          if (filepathFilter.slice(-5) === ".html") {
            return (
              '<li class="list-group-item"><a class="btn btn-link" href="' +
              filepathFilter +
              '">' +
              filepathFilter +
              "</a></li>"
...
// Copy runtime assets that must survive a clean build.
function staticAssets() {
  return gulp.src("src/static/**/*").pipe(gulp.dest("build"));
}
function galleryVendor() {
  return gulp.src("src/js/vendor/gallery.js").pipe(gulp.dest("build/js"));
}
function layoutData() {
  return gulp.src("src/layouts/*.json").pipe(gulp.dest("build"));
}
async function copyImages() {
  const files = glob.sync("src/images/**/*.{png,jpg,jpeg}");
  for (const file of files) {
    const output = path.join("build/images", path.relative("src/images", file));
    await fs.ensureDir(path.dirname(output));
    const image = sharp(file);
    if (/\.png$/i.test(file)) image.png({ palette: true, quality: 90 });
...
async function responsiveImages() {
  ...
  for (const image of images) {
    for (const width of image.widths) {
      await sharp("src/images/content/" + image.source)
        .resize({ width, withoutEnlargement: true })
        .webp({ quality: 82 })
        .toFile("build/images/content/" + image.name + "-" + width + ".webp");
    }
  }
}
...
const compile = gulp.series(
  gulp.parallel(scripts, galleryVendor, twigTemplates, staticAssets, layoutData),
  gulp.parallel(css, cssPrint),
  htmlBeauty,
  indexInject
);
gulp.task("default", gulp.series(
  gulp.parallel(copyImages, responsiveImages, svgMin, copyFavicons, copyFonts),
  compile,
...
gulp.task("build-svg", gulp.series(svgMin));
gulp.task("optimize-images", gulp.series(tinyPngImages, tinyPngImagesFavicon));
